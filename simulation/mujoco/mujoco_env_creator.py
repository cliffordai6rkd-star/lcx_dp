...
class MujocoEnvCreator:
    ...
    def _load_config(self) -> Dict:
        ...
        if not config_path.is_absolute():
            config_path = os.path.join(self.root_path, config_path)
    ...
    def _add_robot_assets_and_worldbody(self, robot_config, root: ET.Element, worldbody: ET.Element) -> None:
...
def main():
    ...
    try:
        ...
        print("Available robots:", [d.name for d in os.scandir((os.path.join(env_creator.root_path, "assets")))
                                  if d.is_dir() and not d.name.startswith("scene")])
        print("Available objects:", [d.name for d in os.scandir((os.path.join(env_creator.root_path, "assets", "scene_objects")))
                                   if d.is_dir()])

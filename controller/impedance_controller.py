...
class ImpedanceController(ControllerBase):
    ...
    def compute_controller(self, target: list[dict[str, np.ndarray]], 
        ...
        self._robot_model.update_kinematics(robot_state._positions, 
        ...
        if self._pose_error_clip:
            pose_error = np.clip(pose_error, 
                        self._pose_error_clip["min"],
                        self._pose_error_clip["max"])
        ...
        task_inertial = np.linalg.pinv(task_inertial_inv + 1e-4 * np.eye(6))
        ...
        C = self._robot_model.get_coriolis_matrix(robot_state._positions, 
                        robot_state._velocities, dims=self.arm_joint_idxes)
        ...
        if self.Kp_nullspace is not None and self.q_des is not None:
            ...
            null_err = np.diag(self.Kp_nullspace) @ (self.q_des - robot_state._positions) \
                        - np.diag(dq_damping) @ robot_state._velocities
        ...
        # print(f'max tau id:{np.argmax(tau)}')

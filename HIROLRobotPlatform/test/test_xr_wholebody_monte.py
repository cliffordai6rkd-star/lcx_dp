...
def main():
    ...
    model_config = "motion/config/robot_model_cfg_monte_01_dual.yaml"
    model_cfg = get_cfg(model_config)["monte_01_dual"]
    ...
    duo_ik = WholeBodyIk(controller_config, model)
    
    # mujoco 
    mujoco_cfg = "simulation/config/mujoco_monte_01.yaml"
    mujoco_cfg = get_cfg(mujoco_cfg)["mujoco"]
    mujoco = MujocoSim(mujoco_cfg)
    world2base, base2world = get_sim_base_world_transform(mujoco)
    log.info(f'world2base: {world2base}')
    ...
    log.info(f'model total dof: {total_dof}')
        ...
        if data[0]:
            ...
            for i, (key, value) in enumerate(arm_target.items()):
                ...
                if tool_target[key][-1] and not enabled and inteface_mode == "absolute_delta":
                    if i == 0: init_ee_pose = []
                    model_type = "single"
                    cur_ee_pose = model.get_frame_pose(frames[i], joint_position, True, model_type)

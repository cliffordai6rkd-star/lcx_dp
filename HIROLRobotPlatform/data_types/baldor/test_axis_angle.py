...
class TestModule(unittest.TestCase):
  def test_to_euler(self):
    ai, aj, ak = br_axis_angle.to_euler([1, 0, 0], 0, axes='sxyz')
    np.testing.assert_allclose([ai, aj, ak], 0, rtol=1e-7, atol=1e-8)
    ai, aj, ak = br_axis_angle.to_euler([0, 1, 0], 1.5, axes='sxyz')
    np.testing.assert_allclose([ai, aj, ak], [0, 1.5, 0], rtol=1e-7, atol=1e-8)

  def test_to_quaternion(self):
    q = br_axis_angle.to_quaternion([1., 0., 0.], np.pi, isunit=True)
    np.testing.assert_allclose(q, [0., 1., 0., 0.], rtol=1e-7, atol=1e-8)
    q = br_axis_angle.to_quaternion([2, 0, 0], np.pi, isunit=False)
    np.testing.assert_allclose(q, [0., 1., 0., 0.], rtol=1e-7, atol=1e-8)

  def test_to_transform(self):
    # Know transform
    T = br_axis_angle.to_transform([0, 0, 1], np.pi / 2, [1, 0, 0])

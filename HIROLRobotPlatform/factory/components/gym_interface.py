...
def _get_episode_writer_cls():
    module = import_module("dataset.lerobot.data_process")
    return getattr(module, "EpisodeWriter")
...
class GymApi(gym.Env):
    def __init__(self, config):
        ...
        # collision_related
        self._collision_height_thresh = config.get("height_thresh", -0.1)
        ...
        self._record_task_dir = None
        self._is_recording = False
        self._episode_id = None
        if self._data_dir:
            cur_path = os.path.dirname(os.path.abspath(__file__))
            self._record_task_dir = os.path.join(cur_path, "../..", self._data_dir)
    ...
    def step(self, action, update_obs=True):
        ...
        if self._data_recorder:
            record_actions = {
                ...
                "arm": np.asarray(execute_arm_action).copy(),
                "tool": np.asarray(action["tool"]).copy(),
            }
            self.record_current_snapshot(
                observation=observation,
                capture_observation_if_missing=False,
    ...
    def get_observation(self):
        if self._obs_type == ObservationType.IMG_ONLY:
            ...
            obs_dict = {'state': {},
                    'colors': camera_data["color"], 
                    'depths': camera_data["depth"]}
            return obs_dict
        else :
            log.info(f"Has lowdim observation")
    ...
    def start_recording(self):
        if self._data_recorder is None and self._record_task_dir:
            episode_writer_cls = _get_episode_writer_cls()
            self._data_recorder = episode_writer_cls(task_dir=self._record_task_dir, rerun_log=False)
    ...
    def add_record_data(self, ee_states=None, joint_states=None, colors=None,
                        depths=None, imus=None, tools=None, actions=None):
        ...
        tools = self.get_tool_state() if tools is None else tools
        colors = self._normalize_media_dict(colors)
        depths = self._normalize_media_dict(depths)
        self._data_recorder.add_item(
            colors=colors,
            depths=depths,
    ...
    def record_current_snapshot(self, observation=None, ee_states=None, joint_states=None,
                                tools=None, actions=None, capture_observation_if_missing=True):
        if observation is None and capture_observation_if_missing:
            observation = self.get_observation()

        colors = None if observation is None else observation.get("colors")
        depths = None if observation is None else observation.get("depths")
        return self.add_record_data(
            ee_states=ee_states,
            joint_states=joint_states,
            colors=colors,
            depths=depths,
    ...
    def _normalize_media_dict(self, media_dict):
        if not media_dict:
            return media_dict

        default_ts = self.get_obs_timestamp()
        normalized_media = {}
        for key, value in media_dict.items():
            if isinstance(value, dict) and "data" in value:
                normalized_value = dict(value)
                if "time_stamp" not in normalized_value:
                    normalized_value["time_stamp"] = normalized_value.get("timestamp", default_ts)
                normalized_media[key] = normalized_value
            else:
                normalized_media[key] = {
                    "data": value,
                    "time_stamp": default_ts,
                }

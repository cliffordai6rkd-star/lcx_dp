...
class MotionFactory:
    ...
    def __init__(self, config, robot: RobotFactory):
        ...
        self._controller_classes = {
            ...
            'g1_ik': "controller.g1_controller:G1Controller",
        }
    ...
    def create_motion_components(self):
        ...
        if self._use_traj_planner:
            ...
            trajectory_cls = _resolve_class(self._trajectory_classes[self._trajectory_planner_type])
            self._trajectory = trajectory_cls(
        ...
        model_cls = _resolve_class(self._model_classes[self._model_type])
        self._robot_model = model_cls(model_config[model_name])
        
        if not object_class_check(self._controller_classes, self._controller_type):
            raise ValueError
        controller_config = self._config["controller_config"]
        controller_cls = _resolve_class(self._controller_classes[self._controller_type])

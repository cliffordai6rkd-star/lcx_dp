...
class EpisodeWriter():
        ...
        while not self.stop_worker or not self.item_data_queue.empty():
            ...
            try:
                ...
                try:
                    ...
                    log.exception(f"Error processing item_data (idx={item_data['idx']})")
                finally:
    ...
    def _add_images_to_item_data(self, idx, image_data, item_data, save_dir, image_desc, attribute_name=None):
        for id, (image_key, image_value) in enumerate(image_data.items()):
            image_array, time_stamp = self._normalize_image_entry(image_value, attribute_name=attribute_name)
            extension = '.jpg' if '_color' in image_key else '.png'
            image_name = f'{str(idx).zfill(6)}_{image_key}{extension}'
            if not cv2.imwrite(os.path.join(save_dir, image_name), image_array):
    ...
    def _normalize_image_entry(self, image_value, attribute_name=None):
        if isinstance(image_value, dict):
            if attribute_name is not None:
                image_array = image_value.get(attribute_name, image_value.get("data"))
            else:
                image_array = image_value.get("data", image_value)
            time_stamp = image_value.get("time_stamp", image_value.get("timestamp", 0.0))
        else:
            image_array = image_value
            time_stamp = 0.0

        return image_array, time_stamp

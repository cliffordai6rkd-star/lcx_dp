...
class RerunEpisodeReader:
    ...
    def return_episode_observations(self, episode_idx, skip_steps_nums=1):
        ...
        if async_save_ft:
            log.info(f'Async save ft files: {async_ft_files}')

        episode_data = []
        counter = 0
        data_type_validated = False
        ...
        for i, item_data in enumerate(json_data):
            ...
            if len(init_ee_poses) == 0:
                for key, cur_ee_state in ee_states.items():
                    if self._state_keys and key not in self._state_keys:
                        continue
                ...
                if self._obs_type in joint_check:
                    ...
                    if self._obs_type == ObservationType.JOINT_POSITION_END_EFFECTOR:
                        ee_pose = self.apply_rotation_offset(
                            ee_states[key]["pose"], key, init_data=init_ee_poses[key]
                        )
                    ...
                    if self._obs_type == ObservationType.END_EFFECTOR_POSE:
                        ...
                        last_pose = self.apply_rotation_offset(last_pose, key, init_data=init_ee_poses[key])
            ...
            if self._state_keys is None:
                self._state_keys = found_obs_keys
            ...
            if tool_states is None or len(tool_states) == 0:
                log.warn('Dataset do not contain tool infos, please double confirm whether you are training a policy without tool usage!!!')
            else:

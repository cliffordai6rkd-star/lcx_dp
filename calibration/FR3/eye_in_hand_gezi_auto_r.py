...
#         # 如果是第一次移动，保存初始旋转作为参考
#         if 'initial_rotation' not in globals():
...
#             try:
#                 # 获取当前旋转矩阵
#                 R_current = new_pose[:3, :3]
                
#                 # 使用真正的双向随机角度，允许正负旋转
#                 # 随着尝试次数增加，减小范围
#                 max_angle = 0.15 - 0.03 * attempt  # 从±9度开始，逐渐缩小
#                 max_angle = max(0.02, max_angle)  # 确保至少有±1.2度的随机性
...
#                 # 应用随机旋转
#                 R_new = R_current @ R_random
                
#                 # 计算相对于初始位置的累积旋转
#                 R_relative = initial_rotation.T @ R_new
                
#                 # 使用更精确的方法计算旋转角度，可以区分正负
#                 # 提取旋转矩阵的旋转轴和角度
#                 rot_trace = np.trace(R_relative)
#                 rot_trace = min(3.0, max(-1.0, rot_trace))  # 避免数值误差
#                 rotation_angle = np.arccos((rot_trace - 1) / 2)
#                 rotation_angle_deg = np.degrees(rotation_angle)
                
#                 # 检查旋转是否在安全范围内 (±75度)
#                 max_allowed_angle = 75.0
#                 if rotation_angle_deg > max_allowed_angle:
#                     print(f"尝试 {attempt+1}: 累积旋转 {rotation_angle_deg:.1f}° 超过安全阈值 {max_allowed_angle}°，重新生成")
#                     continue
...
#                 print(f"相对于初始位置的累积旋转: {rotation_angle_deg:.1f}°")
...
#         # 如果所有尝试都失败，返回初始姿态，只改变位置
#         print("所有随机旋转尝试都失败，尝试使用初始姿态...")
#         try:
#             reset_pose = np.eye(4)
#             reset_pose[:3, :3] = initial_rotation  # 使用初始旋转
...
#             new_pose[:3, 3] = point
#             panda.move_to_pose(new_pose)
#             print(f"纯位移成功: X={point[0]:.3f}, Y={point[1]:.3f}, Z={point[2]:.3f}")
#             return True

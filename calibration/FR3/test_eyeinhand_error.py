...
import time
# 眼在手上标定矩阵（从cam到 夹抓的变换）
# T_cam_to_gripper = np.array([
# [ 0.01442727 ,-0.96976586 , 0.2436104 ,  0.02130021],
#  [ 0.99887669 , 0.0029805 , -0.04729136 ,-0.04250526],
#  [ 0.04513547 , 0.24401903,  0.96871952 ,-0.12341386],
#  [ 0.    ,      0. ,         0. ,         1.        ]
# ])
T_cam_to_gripper = np.array(
 [[ 2.11634506e-03, -9.99985441e-01, -4.96375115e-03 , 3.00671720e-02],
 [ 9.99964802e-01,  2.07594935e-03,  8.12922326e-03 , 2.78860658e-04],
 [-8.11880041e-03 ,-4.98078068e-03,  9.99954637e-01 ,-5.65942992e-02],
 [ 0.00000000e+00 , 0.00000000e+00 , 0.00000000e+00 , 1.00000000e+00]]
)

# # 中间节点
# pose1 = np.array(
# [[ 0.87889901,  0.4744626  ,-0.04921111 , 0.45991142],
#  [ 0.47553845, -0.87960779,  0.01238099 ,-0.0523434 ],
#  [-0.03741216 ,-0.03428342, -0.99871166 , 0.20334544],
#  [ 0.  ,        0. ,         0. ,         1.        ]]
#     )
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # # 【眼在手上处理流程】
        # # 1. 获取当前机器人末端姿态（从基座到夹爪的变换）
        # T_base_to_gripper = panda.get_pose()  # 获取当前机器人姿态矩阵
        # print("当前机器人姿态 (基座到夹爪):", T_base_to_gripper)

        # # 2. 将相机坐标转换为夹爪坐标
        # point_camera_hom = np.append(point_camera, 1.0)
        # point_gripper_hom = T_cam_to_gripper @ point_camera_hom
        # point_gripper = point_gripper_hom[:3] / point_gripper_hom[3]
        # print("夹爪坐标系下的 3D 点:", point_gripper)

        # # 3. 将夹爪坐标转换为基座坐标
        # point_gripper_hom = np.append(point_gripper, 1.0)
        # point_base_hom = T_base_to_gripper @ point_gripper_hom
        # point_base = point_base_hom[:3] / point_base_hom[3]
        # print("基坐标系下的 3D 点:", point_base)
        # 1. 像素→相机系三维点
        depth = current_depth_frame.get_distance(x, y)
        p_cam = np.array(rs.rs2_deproject_pixel_to_point(intrinsics, [x, y], depth))
        p_cam_hom = np.append(p_cam, 1.0)

        # 2. 取末端在基座下的变换
        T_base_gripper = panda.get_pose()  # 4×4
        print(f'2')
        print("T_base_gripper:\n", T_base_gripper)

        # 3. 相机在基座系下的完整位姿
        T_base_cam = T_base_gripper @ T_cam_to_gripper
        print(f'3')
        print("相机在基座系下的完整位姿:\n", T_base_cam)
        ...
        print(f'4')
        print("该点在基座系下的位置：", p_base)

        # # 在图像上标记点击位置
        # cv2.circle(current_color_frame, (x, y), 5, (0, 0, 255), -1)
        # cv2.putText(current_color_frame, "Press 'g' to move", (x+10, y+10), 
        #            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        # cv2.imshow('Color Frame', current_color_frame)

        # # 询问是否要移动到该位置
        # print("按 'g' 键移动到点击位置，按其他键取消")
        # key = cv2.waitKey(0)  # 等待按键
        # print("ggggg")
        # if key == ord('g') or key == ord('G') or key & 0xFF == ord('g') or key & 0xFF == ord('G'):
        #     try:
        #         # 在mouse_callback的if key == ord('g'):部分
        #         print("按下G键，准备移动")
        #         move_to_clicked_point(p_base)

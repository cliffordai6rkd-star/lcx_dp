...
from quaternion_to_matrix import quaternion_to_matrix, robot_pose_to_matrix
...
component_type = 2
robot.set_arm_enable(component_type, 1)
robot.set_arm_mode(component_type, 2)
robot.set_arm_state(component_type, 0)
...
try:
    while True:
        ...
        if key == ord('r') and homogeneous_matrix is not None:
            ...
            # 获取末端位姿
            success, matrix = robot.get_arm_position(component_type)
            if success:
                matrix = robot_pose_to_matrix(matrix)
            ...
            # 构造图像保存路径
            image_filename = os.path.join(current_dir, f'image_{image_counter}.png')
